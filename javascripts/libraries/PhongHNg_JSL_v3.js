...
const PhongHNg_JSL = {
    ...
    /* Convert from array format by rule to DOM format */
    ADOM(array, document) {
        let [tag_name, attributes, inner_HTML, childeren] = array;
        ...
        if (childeren)
            for (let child_array of childeren)
                DOM.appendChild(PhongHNg_JSL.ADOM(child_array, document));
        return DOM;
    }
}